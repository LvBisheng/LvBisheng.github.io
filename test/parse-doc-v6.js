...
<head>
    ...
    <style>
        ...
        .level-3 {
          ...
          min-width: 1.5em;    /* 给短序号一个基础宽度，保证稍微对齐 */
          margin-right: 8px;   /* 核心！强制在序号右侧留出 8px 的安全距离 */
        }
...
try {
  ...
  let result = html
    ...
    // 6. 替换特殊空格为普通空格
    .replace(/(&nbsp;|&#9;)+/gi, ' ')
    // 7. 清理空标签、释放被包裹的空格，并合并连续加粗 (✅ 核心修复区)
    .replace(/<b>(\s+)<\/b>/gi, '$1') // 提取仅包含空格的 b 标签内的空格
    .replace(/<b><\/b>/gi, '')        // 清除纯空的无用 b 标签
    .replace(/<\/b>(\s*)<b>/gi, '$1') // 合并相邻的 b 标签，并保留它们中间的空格
    .replace(/<p>\s*<\/p>/gi, '')     // 清理空段落
    // 8. 处理 Word 的列表符号
